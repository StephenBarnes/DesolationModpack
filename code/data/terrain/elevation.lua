...
local function getStartIslandAndOffshootsMinElevation(scale, x, y, tile, map)
	...
	elevation = noise.if_else_chain(
		noise.less_than(elevation, -10),
		elevation,
		noise.less_than(0, elevation),
		elevation,
		-0.5) -- Set elevation to -0.5 where it would have been between -10 and 0, so that we place shallow water.
...
local function getOtherIslandsMinElevation(scale, x, y, tile, map)
	...
	local elevation = U.multiBasisNoise(8, 2, 2, 1 / (C.otherIslandNoiseScale * scale), C.otherIslandNoiseAmplitude) - 11.0
