...
local standard_starting_lake_elevation_expression = noise.define_noise_function(function(x, y, tile, map)
	...
	return noise.ident(noise.min(
		...
		starting_cone_offset + starting_cone_slope * starting_lake_distance +
		starting_cone_noise_multiplier * starting_lake_noise,
		noise.max(
			second_cone_offset,
			second_cone_offset + second_cone_slope * starting_lake_distance +
			second_cone_noise_multiplier * starting_lake_noise
		)
	))
end)
...
local function IS_make_lakes(x, y, tile, map, options)
	...
	local empty_radius = 2000
	local avoid_starting_island = function(dist)
		return noise.clamp((dist - empty_radius) / (empty_radius + 700), 0, 1) ^ 3
	end
