-- This file allows generating Intermodal Containers packing recipes for item subclasses where IC currently doesn't allow it.
-- This file's code is copied from __IntermodalContainers__/prototypes/shared.lua and __IntermodalContainers__/prototypes/utils.lua.
-- I think copying this code here wouldn't be necessary if IC changes the import at the top of shared.lua, from "prototypes.utils" to "__IntermodalContainers__.prototypes.utils" or just "utils".

-- The following doesn't quite work, because the require'd file itself requires "prototypes.utils", which it searches for in Desolation and in __IntermodalContainers__.prototypes.prototypes.utils (note repeated "prototypes").
--local IC = require "__IntermodalContainers__.prototypes.shared"
--for _, type in pairs(typesForPacking) do
--	for _, item in pairs(data.raw[type]) do
--		IC.generate_crates(item)
--	end
--end

local utils = require "copied-utils"
...
IC.CRATE_ORDER = 0
function IC.generate_crates(this_item, typeOrNil)
  -- The crated item
  -- NOTE: StephenB added the typeOrNil param to eliminate some redundancy. This is not in base Intermodal Containers.
  --local base_item = utils.get_item_prototype(this_item)
  local base_item
  if typeOrNil == nil then
    base_item = utils.get_item_prototype(this_item)
  else
    base_item = data.raw[typeOrNil][this_item]
  end
  if not base_item then
    log("ERROR: IC asked to crate an item that doesn't exist ("..serpent.line(this_item)..")")
    return
  end
